...
PetAI.VERSION = "6.4.1"  -- Mascotas inteligentes mejoradas
PetAI.ENABLED = true
PetAI.debug = false
PetAI.lastUpdate = 0
...
-- ============================================================================
-- SISTEMA DE MODOS DE IA (Agresivo, Defensivo, Soporte)
-- ============================================================================
PetAI.currentMode = 1  -- 1=Agresivo, 2=Defensivo, 3=Soporte
...
function PetAI:SetMode(mode)
    if not mode or mode < 1 or mode > 3 then
        self:Print("Modo inválido. Usa 1 (Agresivo), 2 (Defensivo) o 3 (Soporte)")
        return false
    end
...
function PetAI:ExecuteAbility(spellName)
    if self:IsOnCooldown(spellName) then return false end
...
function PetAI:Evaluate()
    ...
    if self:IsEnslavedDemon() then
        if self:EvaluateEnslaved() then return end
    end
...
PetAI:Print("v" .. PetAI.VERSION .. " - Sistema de IA con Auto-Reenslave")
