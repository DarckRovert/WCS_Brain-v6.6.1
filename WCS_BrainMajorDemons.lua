...
--[[
WCS_BrainMajorDemons.lua v6.4.2
...
function MajorDemons:DetectMajorDemon()
    ...
    while i <= 10 do
        ...
        if name then
            ...
            if string.find(name, "pisoteo") then
                return "Doomguard"
            end
...
function MajorDemons:EvaluateInfernal()
    ...
    -- Aviso de expiracion (30 segundos antes)
    if timeRemaining > 0 then
        if timeRemaining < 30 then
            if not self.State.warningShown then
                self.State.warningShown = true
                self:Print("|cffff0000AVISO:|r Infernal expira en " .. string.format("%.0f", timeRemaining) .. " segundos!")
            end
        end
    end
...
function MajorDemons:EvaluateDoomguard()
    ...
    if timeRemaining > 0 then
        if timeRemaining < 30 then
            if not self.State.warningShown then
                self.State.warningShown = true
                self:Print("|cffff0000AVISO:|r Doomguard expira en " .. string.format("%.0f", timeRemaining) .. " segundos!")
            end
        end
    end
